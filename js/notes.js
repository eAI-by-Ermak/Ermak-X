...
(function () {
  ...
  function navigateErmak(url) {
    if (window.ErmakApp && window.ErmakApp.navigate) window.ErmakApp.navigate(url);
  }
  ...
  async function loadNotes() {
    try { const v = await idbGet('ermakx_notes'); notes = Array.isArray(v) ? v : []; } catch (e) { notes = []; }
  }
  ...
  async function init() {
    ...
    await loadNotes();
    renderList();
    const homeBtn = document.getElementById('notesHomeBtn');
    if (homeBtn) homeBtn.addEventListener('click', () => navigateErmak('home'));
